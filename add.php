...
if ( !$connection ) {
    ...
    if ( 'addManager' == $action ) {
        ...
        if ( $fname && $lname && $lname && $phone && $password ) {
            $hashPassword = password_hash( $password, PASSWORD_BCRYPT );
            $query = "INSERT INTO customers(fname,lname,email,phone,password) VALUES ('{$fname}','$lname','$email','$phone','$hashPassword')";
            mysqli_query( $connection, $query );
        ...
        if ( $fname && $lname && $lname && $phone ) {
            $query = "UPDATE customers SET fname='{$fname}', lname='{$lname}', email='$email', phone='$phone' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allCustomer" );
        }
        ...
        if ( $serialno && $name && $price && $salesman ) {
            $query = "INSERT INTO drugs(serialno,name,price,salesman) VALUES ('{$serialno}','$name','$price','$salesman')";
            mysqli_query( $connection, $query );
        ...
        if ( $serialno && $name && $price && $salesman ) {
            $query = "UPDATE drugs SET serialno='{$serialno}', name='{$name}', price='$price', salesman='$salesman' WHERE id='{$id}'";
            mysqli_query( $connection, $query );
            header( "location:index.php?id=allDrug" );
        }
